       ...
       // bool hasSetColumn_PT = false;
        ...
        private void LoadLaserInfo(string strParkNo, int laserActionCount, ParkLaserOut park)
        {
            ...
            try
            {
                ...
                //    return;
                //}
                if ( comb_ParkingNO.Text.Trim() == "请选择")
                {
                    return;
                }
                dtParksize.Clear();
                ...
                int XMin, XMax, YMin, YMax, XGroove, YGroove;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // && comb_ParkingNO.Text.Trim().Contains('Z')
            if (comb_ParkingNO.Text.Trim() != "请选择" && comb_ParkingNO.Text.Trim() != "")
            {
               ...
               if (auth.IsOpen("框架车出库材料选择"))
               {
                   ...
                   auth.OpenForm("框架车出库材料选择", parkNO);
               }
               else
                   auth.OpenForm("框架车出库材料选择", parkNO);
            }
        ...
        private void btnSelectCoilUpCar_Click(object sender, EventArgs e)
        {
            FrmSelectCoilUpCar newFrm = new FrmSelectCoilUpCar();
            newFrm.packing.PackingNo = comb_ParkingNO.Text.Trim();
            newFrm.packing.STOWAGE_ID = txt_LoadMapID.Text.Trim();
            newFrm.packing.CarNo = txt_CarNO.Text.Trim();
            newFrm.packing.TREATMENT_NO = txt_TreatmentNO.Text.Trim();
            newFrm.Show();
        }
