...
namespace CONTROLS_OF_REPOSITORIES
{
    public partial class FrmSaddleShow : Form
    {
        ...
        //20200509添加2-----yezhiwei
        private SaddleInBay saddle = new SaddleInBay();
        ...
        //20200509添加按钮-----叶志伟
        private void double_deck_Click(object sender, EventArgs e)
        {
            //SaddleInBay saddle = new SaddleInBay();
            //conSaddle saddle = new conSaddle();
            if (double_deck.Text == "显示双层库位")
            {
                saddle.conInit(areaBase.AreaNo, SaddleBase.tagServiceName);
                saddle.Double_deck = true;
                refreshSaddle();
                double_deck.Text = "隐藏双层库位";
                
            }
            else
            {
                saddle.conInit(areaBase.AreaNo, SaddleBase.tagServiceName);
                saddle.Double_deck = false ;
                refreshSaddle();
                double_deck.Text = "显示双层库位";
