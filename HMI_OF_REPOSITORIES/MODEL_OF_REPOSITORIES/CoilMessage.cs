    ...
    public  class CoilMessage
    {
        ...
        public void GetCoilMessageByCoil(DataGridView dgv, string coilNo,string plantType)
        {
            ...
            try
            {
                ...
                if (plantType=="入库") //入库
                {
                    sql = @"SELECT DECODE(B.PLAN_TYPE ,0,'出库',1,'入库') AS PLAN_TYPE1 ,A.REC_TIME AS REC_TIME1, '' AS PLAN_TYPE2, '' AS REC_TIME2,
                     B.PLAN_NO, D.STOCK_NO, C.COIL_NO ,C.WEIGHT,C.WIDTH,C.INDIA,C.OUTDIA, C.PACK_FLAG, C.SLEEVE_WIDTH, 
                     C.COIL_OPEN_DIRECTION, C.NEXT_UNIT_NO,STEEL_GRANDID, C.ACT_WEIGHT, C.ACT_WIDTH,G.LOGISTICS_FLAG FROM UACS_YARDMAP_STOCK_DEFINE D
                     RIGHT JOIN UACS_PLAN_IN_DETAIL A ON A.MAT_NO = D.MAT_NO
                      RIGHT JOIN UACS_PLAN_IN B ON B.PLAN_NO = A.PLAN_NO
                      LEFT JOIN  UACS_YARDMAP_COIL C  ON D.MAT_NO = C.COIL_NO 
                      LEFT JOIN UACS_LOGISTICS_CONFIG G ON A.HAVEN_CNAME = G.HAVEN_CNAME  ";
                  
                    sql += "  WHERE COIL_NO LIKE '%" + coilNo + "%' ";
                }
                else if(plantType=="出库")
                {
                    sql = @" SELECT DECODE(B.PLAN_TYPE ,0,'出库',1,'入库') AS PLAN_TYPE2 ,A.REC_TIME AS REC_TIME2,'' AS PLAN_TYPE1, '' AS REC_TIME1, 
                     B.PLAN_NO, C.COIL_NO ,D.STOCK_NO, C.WEIGHT,C.WIDTH,C.INDIA,C.OUTDIA, C.PACK_FLAG, C.SLEEVE_WIDTH,  
                     C.COIL_OPEN_DIRECTION, C.NEXT_UNIT_NO,STEEL_GRANDID, C.ACT_WEIGHT, C.ACT_WIDTH,G.LOGISTICS_FLAG FROM UACS_YARDMAP_STOCK_DEFINE D 
                     RIGHT JOIN UACS_PLAN_OUT_DETAIL A ON A.MAT_NO = D.MAT_NO 
                     RIGHT JOIN UACS_PLAN_OUT B ON B.PLAN_NO = A.PLAN_NO 
                     LEFT JOIN  UACS_YARDMAP_COIL C ON D.MAT_NO = C.COIL_NO  
                     LEFT JOIN UACS_LOGISTICS_CONFIG G ON B.HAVEN_ID = G.HAVEN_CNAME  ";

                    sql += "  WHERE D.MAT_NO  LIKE  '%" + coilNo + "%' ";
                }
                else
                {
                    sql = @"SELECT DECODE(B.PLAN_TYPE ,0,'出库',1,'入库') AS PLAN_TYPE1 ,A.REC_TIME AS REC_TIME1, DECODE(E.PLAN_TYPE ,0,'出库',1,'入库') AS PLAN_TYPE2 ,D.REC_TIME AS REC_TIME2,
                    B.PLAN_NO, C.COIL_NO, F.STOCK_NO, C.WEIGHT,C.WIDTH,C.INDIA,C.OUTDIA, C.PACK_FLAG, C.SLEEVE_WIDTH, 
                    C.COIL_OPEN_DIRECTION, C.NEXT_UNIT_NO,STEEL_GRANDID, C.ACT_WEIGHT, C.ACT_WIDTH,G.LOGISTICS_FLAG FROM UACS_YARDMAP_STOCK_DEFINE F 
                    LEFT JOIN UACS_YARDMAP_COIL C ON F.MAT_NO = C.COIL_NO 
                    LEFT JOIN UACS_PLAN_IN_DETAIL A ON A.MAT_NO = C.COIL_NO 
                    LEFT JOIN UACS_PLAN_IN B ON B.PLAN_NO = A.PLAN_NO 
                    LEFT JOIN UACS_PLAN_OUT_DETAIL D ON D.MAT_NO = C.COIL_NO 
                    LEFT JOIN UACS_PLAN_OUT E ON E.PLAN_NO = D.PLAN_NO
                    LEFT JOIN UACS_LOGISTICS_CONFIG G ON A.HAVEN_CNAME = G.HAVEN_CNAME ";

                    sql += "  WHERE F.MAT_NO LIKE '%" + coilNo + "%' ";
                }
