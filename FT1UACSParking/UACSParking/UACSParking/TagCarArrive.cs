    ...
    public partial class TagCarArrive : Form
    {
        ...
        private void setScanCrane(string parkNO, int craneNO)
        {
            try
            {
                ...
                UACSUtility.HMILogger.WriteLog(btnCreate.Text, "库位扫描行车修改，停车位：" + parkNO + "行车：" + craneNO, UACSUtility.LogLevel.Warn, this.Text, ((IAuthorization)FrameContext.Instance.GetPlugin<IAuthorization>()).GetUserName());
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message + "\r\n" + ex.StackTrace);
