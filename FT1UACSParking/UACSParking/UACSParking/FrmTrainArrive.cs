    ...
    public partial class FrmTrainArrive : Form
    {
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ClsParkingManager.TagDP.SetData(ClsParkingManager.TAG_EV_RAILWAY_COACH_ARRIVE, tagValue); //PT55|2
                UACSUtility.HMILogger.WriteLog(btnConfirm.Text, "火车到位：" + tagValue, UACSUtility.LogLevel.Info, this.Text, ((IAuthorization)FrameContext.Instance.GetPlugin<IAuthorization>()).GetUserName());
        ...
        private void BindRailwayLineNO(string _trainLine)
        {
            ...
            dr["TypeName"] = string.Format("{0}{1}",  _trainLine.Equals("PT55") ?"A跨 ": _trainLine.Equals("PT55B") ? "A跨 " : _trainLine.Contains("PT57")?"C跨 ": "" , _trainLine);
            dt.Rows.Add(dr);
        ...
        private void BindScanCrane(string _trainLine)
        {
            ...
            else if (_trainLine.Equals("PT55B"))
            {
                dr = dt.NewRow();
                dr["TypeValue"] = 3;
                dr["TypeName"] = "3#行车";
