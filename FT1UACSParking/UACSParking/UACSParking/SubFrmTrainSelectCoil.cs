...
namespace UACSParking
{
    ...
    public partial class SubFrmTrainSelectCoil : Baosight.iSuperframe.Forms.FormBase
    {
        ...
        public SubFrmTrainSelectCoil(ClsTrainCase clsTrain)
        {
            ...
            stowageType = clsTrain.StowageType;
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == 0 && e.RowIndex != -1)
                {
                    ...
                    string coilMatNO = "";
                    int Weight = 0;
                    int saddleSeq = Convert.ToInt32(clsCoilSelected.bracketNO.Substring(0, 1));

                    int coilsCountLeft, coilsCountMiddle, coilsCountRight;
                    //ClsTrainCase clstrain = new ClsTrainCase();
                    ...
                    Weight = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["WEIGHT"].Value);
                    ...
                    if (hasCheck && clsCoilSelected.bracketNO != null && clsCoilSelected.bracketNO != "")
                    {
                        ...
                        if(saddleSeq > 0 && saddleSeq <= coilsCountRight)
                        {
                            CoilsWeightR += Weight;
                        }
                        else if(saddleSeq > (coilsCountMiddle + coilsCountRight) && saddleSeq <= (coilsCountLeft + coilsCountMiddle + coilsCountRight))
                        {
                            CoilsWeightL += Weight;
                        }
                        updateWeight();
        ...
        private void btnCommit_Click(object sender, EventArgs e)
        {
            ...
            if (System.Math.Abs(CoilsWeightL - CoilsWeightR) > 2000)
            {
                MessageBox.Show(string.Format("车皮两端钢卷重量差值为{0}kg,超过2吨！", System.Math.Abs(CoilsWeightL - CoilsWeightR).ToString()));
            }
        ...
        void updateWeight()
        {          
            txtCoilsWeightL.Text = string.Format("{0}/kg", CoilsWeightL.ToString());
            txtCoilsWeightR.Text = string.Format("{0}/kg", CoilsWeightR.ToString());
        }
    }
