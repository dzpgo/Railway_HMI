    ...
    public partial class FrmCarEntry : Form
    {
            ...
            if (carType == "普通框架")
            {
                ...
                if (txtCarNo.Text.Trim() != "" || txtDirection.Text.Trim() != "" || txtFlag.Text.Trim() != "" || txtPacking.Text.Trim() != "")
                {
                   ...
                   // sb.Append(carDirection.Trim());
                    ...
                    //    return;
                    //}
                    UACSUtility.HMILogger.WriteLog(btnConfirm.Text, "车到位：" + sb.ToString(), UACSUtility.LogLevel.Info, this.Text, ((IAuthorization)FrameContext.Instance.GetPlugin<IAuthorization>()).GetUserName());
        ...
        private void setScanCrane(string parkNO, int craneNO)
        {
            try
            {
                ...
                UACSUtility.HMILogger.WriteLog(btnConfirm.Text, "库位扫描行车修改，停车位：" + parkNO + "行车：" + craneNO, UACSUtility.LogLevel.Warn, this.Text, ((IAuthorization)FrameContext.Instance.GetPlugin<IAuthorization>()).GetUserName());
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message + "\r\n" + ex.StackTrace);
