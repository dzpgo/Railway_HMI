    ...
    public partial class TruckStowageInT : Baosight.iSuperframe.Forms.FormBase
    {
        ...
        private void Inq_LoadMap(int thestowageId)
        {
            try
            {
                ...
                A.SLEEVE_LENGTH, A.PACKAGE_STATUS , A.STATUS , D.WEIGHT, D.OUTDIA ,D.WIDTH, D.STEEL_GRANDID, D.NEXT_UNIT_NO, D.SLEEVE_WIDTH, C.LOGISTICS_FLAG  FROM UACS_TRUCK_STOWAGE_DETAIL A  ";
            ...
            try
            {
                ...
                if (cancelFlag == false)
                {
                   ...
                   // tagProvider_InStock.SetData("EV_NEW_PARKING_CARARRIVE", theOperator + "|" + date + "|" + shift + "|" + crew + "|" + parkingNO + "|" + carNO + "|" + isLoaded + "|" + headPostion + "|" + pullAbility + "|" + equno + "|" + cartype + "|" + parkingype);
                    tagProvider_InStock.SetData("EV_NEW_PARKING_CARARRIVE", tagCarInStosk);
                    UACSUtility.HMILogger.WriteLog(cmd_CarArrive.Text, "作业开始：" + comb_ParkingNO.Text.Trim(), UACSUtility.LogLevel.Info, this.Text, ((IAuthorization)FrameContext.Instance.GetPlugin<IAuthorization>()).GetUserName());
