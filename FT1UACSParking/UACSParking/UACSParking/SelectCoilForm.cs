    ...
    public partial class SelectCoilForm : Form
    {
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (carType == "101" && txtGetMat.Text.Trim()!="0")                    //开关
                {
                    ...
                    for (int j = 0; j < this.dataGridView2.Rows.Count-1; j++)
                    {
                        ...
                        if (parkingNo.Contains("Z5"))  //成品库库位类型
                        {
                            ...
                            if (dist <= safeDistance)
                            {
                                DialogResult dr = MessageBox.Show(string.Format("{0}槽与{1}钢卷之间距离: {2}mm小于安全距离:{3}mm，继续可能存在危险！", j, j + 1, dist, safeDistance), "警告", MessageBoxButtons.YesNo,MessageBoxIcon.Warning);
                                if (dr == DialogResult.Yes)
                                {
                ...
                {
                    DialogResult dr = MessageBox.Show(string.Format("所选钢卷存在多库位或者卷信息有误：\r\n{0}！继续请先确认库位信息。", temp), "警告", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
               ...
               string sqlText = @"SELECT TREATMENT_NO, LASER_ACTION_COUNT FROM UACS_PARKING_STATUS WHERE PARKING_NO='{0}' ";
                ...
                using (IDataReader rdr = DBHelper.ExecuteReader(sqlText))
                {
                    while (rdr.Read())
                    {
                        ...
                        //if (carType == "100")
                        //{
                        //    for (int i = 0; i < Convert.ToInt32(grooveNum) - Convert.ToInt32(grooveTotal); i++)
                        //    {
