...
namespace UACSParking
{
    public partial class PorductMatManage : FormBase
    {
        ...
        //
        string[] dgvColumnsName = { "GROOVEID", "BAY_NO", "FROM_STOCK_NO", "TO_STOCK_NO", "MAT_NO" };
        string[] dgvHeaderText = {"槽号", "跨别", "起卷库位", "落卷库位", "材料号" };
        ...
        {
            if (cbbPacking.Text.Trim() != "请选择" && cbbPacking.Text.Trim() != "" )
            {
                isStowage = false;
                hasCar = true;
                hasParkSize = false;
        ...
        private string GetTextOnCar(string parking)
        {
            try
            {
                string str = "";
                //txtCarNo.Text = "";
                string sql = string.Format("select CAR_NO,HEAD_POSTION ,TREATMENT_NO ,LASER_ACTION_COUNT from UACS_PARKING_STATUS where PARKING_NO = '{0}' ", parking);
            ...
            {            
                ...
                if (carNo != "" && cbbPacking.Text.Contains('F') && cbbPacking.Text.Trim() != "请选择")
                {
                    ...
                    if (strStowageID != "")
                    {
                        ...
                        string sql = @"select C.GROOVEID,C.MAT_NO as COIL_NO2,A.PICK_NO as PICK_NO,C.X_CENTER as GROOVE_ACT_X ,C.Y_CENTER AS GROOVE_ACT_Y,C.Z_CENTER AS GROOVE_ACT_Z, B.WEIGHT, B.OUTDIA ,D.STOCK_NO, D.LOCK_FLAG  from UACS_TRUCK_STOWAGE_DETAIL C ";
        ...
        /// <param name="e"></param>
        //private void cbBoxPickNo_SelectedIndexChanged(object sender, EventArgs e)
        //{
        ...
        private void txtGetPlanNo_TextChanged(object sender, EventArgs e)
        {
            ...
            //        StringBuilder sbb = new StringBuilder(strPacking);
            //        sbb.Append("-1");
            //        BindMatStock(sbb.ToString(), txtGetPlanNo.Text.Trim());
            //        //
            //    }
            //}
        ...
        private int GetCoilWeight(string coilNo)
        {
            try 
	        {
                int weight = 0;
                string sqlText = @"SELECT WEIGHT FROM UACS_YARDMAP_COIL WHERE COIL_NO ='{0}' ";
                sqlText = string.Format(sqlText, coilNo);
                using (IDataReader rdr = DBHelper.ExecuteReader(sqlText))
                {
                    if (rdr.Read())
                    {
        ...
        /// <returns></returns>
        private int GetCoilRadius(string matNo)
        {
            int Radius=0;
           string  sqlText = @"SELECT OUTDIA FROM UACS_YARDMAP_COIL WHERE COIL_NO = '{0}'";
            sqlText = string.Format(sqlText, matNo);
            using (IDataReader rdr = DBHelper.ExecuteReader(sqlText))
            {
                if (rdr.Read())
                {
        ...
        private int GetgrooveDistence()
        {
            ...
            foreach (DataGridViewRow item in dataGridView2.Rows)
            {
                if (item.Cells["GROOVE_ACT_Y"].Value.ToString() != "" && groove1 == 0)
                {
                    groove1 = Convert.ToInt32( item.Cells["GROOVE_ACT_Y"].Value);
                }
                else if (item.Cells["GROOVE_ACT_Y"].Value.ToString() != "" && groove2 == 0)
                {
                    groove2 = Convert.ToInt32(item.Cells["GROOVE_ACT_Y"].Value);
                    break;
                }
            }
            ret = System.Math.Abs(groove1 - groove2);
            return
                ret;
        }
        ...
        private void btnCarFrom_Click(object sender, EventArgs e)
        {
            ...
            DialogResult dr = MessageBox.Show("确定要对" + cbbPacking.Text.Trim() + "跨进行车离位吗？", "操作提示", btn);
            if (dr == DialogResult.OK)
            {
                ...
                //x_coil1 = 0;
                //x_coil2 = 0;
                //coilDistance.Text = string.Format("  /MM");
                //coilDistance.BackColor = Color.White;
                //
                txtCarHeadToward.Text = "";
                txtCarNo.Text = "";
        ...
        private void RefreshOrderDgv(string parkNO)
        {
            ...
            try
            {
                ...
                SQLOder += " WHERE  CAR_NO IN ( SELECT CAR_NO FROM UACS_PARKING_STATUS WHERE PARKING_NO ='{0}')) ORDER BY GROOVEID ";
        ...
        private void LoadLaserInfo(string strParkNo, ParkLaserOut park) 
        {
            ...
            try
            {
                ...
                //    {
                //        park.CreateCarSize(XMin, YMax, XVehicleLength, YVehicleLength, 0, "W");
                //    }
                //    else
                //        park.CreateCarSize(XMin, YMax, XVehicleLength, YVehicleLength, 0, "E");
                //}
        ...
        private void button1_Click(object sender, EventArgs e)
        {
           ...
           //// //RefreshHMILaserOutData();
           //// //CreatParkSize();
           //// //CreatCarSize("Z53A2", laserZ53A2);
           //// //laserZ53A2.ClearbitM();
           //// string parkingNo = this.cbbPacking.Text.Trim();
           //// string treatmentNo = "";
           //// string stowageNo = "";
           ...
           //// sqlText = string.Format(sqlText, parkingNo);
           ////// Dictionary<string, LASER_DATA> dictLaserOut = new Dictionary<string, LASER_DATA>();
           //// using (IDataReader rdr = DBHelper.ExecuteReader(sqlText))
           //// {
           ////     if (rdr.Read())
           ////     {
           ...
           ////// parkLaserOut1.LaserData= GetLaserOutData(treatmentNo, LASER_ACTION_COUNT);
           ////// parkLaserOut1.DrawParksize()
           //// //parkLaserOut1.DrawSelectLoction(1, 1, 1, 1);
           //// parkLaserOut1.CreatVirtualCarSize(GetLaserOutData(treatmentNo, LASER_ACTION_COUNT));


        }

        //private Dictionary<string, LASER_DATA> GetLaserOutData(string treatmentNo, long LASER_ACTION_COUNT)
        //{
        //    bool bResult = false;
        //    string sqlText;
        //    Dictionary<string, LASER_DATA> dictGrooveIDLaserOut = new Dictionary<string, LASER_DATA>();
        //    try
        //    {
        //        // 获取最新激光扫描数据（从出库激光表里取出激光扫描数据）
        ...
        //            {
        //                i++;
        //                LASER_DATA laseroutData;
        //                laseroutData.GROOVE_ACT_X = rdr["GROOVE_ACT_X"].ToString();
        //                laseroutData.GROOVE_ACT_Y = rdr["GROOVE_ACT_Y"].ToString();
        //                laseroutData.GROOVE_ACT_Z = rdr["GROOVE_ACT_Z"].ToString();
        //                laseroutData.GROOVEID = rdr["GROOVEID"].ToString();

        //                dictGrooveIDLaserOut[i.ToString()] = laseroutData;
        ...
        {
            //if (e.ColumnIndex == dgvOrder.Columns["FROM_STOCK_NO"].Index ||
            //    e.ColumnIndex == dgvOrder.Columns["TO_STOCK_NO"].Index)
            //{
            //    string str1 = dgvOrder.Rows[e.RowIndex].Cells["FROM_STOCK_NO"].Value.ToString();
            //    string str2 = dgvOrder.Rows[e.RowIndex].Cells["TO_STOCK_NO"].Value.ToString();
            //    string str3 = dgvOrder.Rows[e.RowIndex].Cells["MAT_NO"].Value.ToString();
            //    if (str1=="" || str2=="" && str3!="")
            //    {
            //        e.CellStyle.BackColor = Color.Red;                       
            //    }
