...
{
    public partial class FrmStockStrategyManageOut : Baosight.iSuperframe.Forms.FormBase
    {
        string craneNO = "1";
        string areaNO = "";
        
        public FrmStockStrategyManageOut()
        {
            InitializeComponent();
            this.Load += FrmStockStrategyManageOut_Load;
        }

        void FrmStockStrategyManageOut_Load(object sender, EventArgs e)
        {
        ...
        void dgvStrategyDetail_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if ( e.RowIndex >= 0 && senderGrid.Rows[e.RowIndex].Cells["ID2"].Value != null )
            {
                ...
                smallArea.areaID = UACSUtility.ViewHelper.JudgeStrNull(senderGrid.Rows[e.RowIndex].Cells["COIL_STRATEGY_ID"].Value);
                smallArea.enable = UACSUtility.ViewHelper.JudgeStrNull(senderGrid.Rows[e.RowIndex].Cells["FLAG_ENABLED2"].Value) == "1" ? true : false;
                smallArea.logisticsFlag = "0";//UACSUtility.ViewHelper.JudgeStrNull(senderGrid.Rows[e.RowIndex].Cells["FLOW_TO2"].Value);
        ...
        private void BindCarType(ComboBox cmbBox ,string bayNO)
        {
         ...
         //private string GetOperateAreaByBay(string craneNO)
         //{
         //    string Area = "";
         //    try
         //    {
         ...
         //            Area = "产成品A-1";
         //        }
         //        else if (craneNO.Contains("2"))
         //        {
         //            Area = "产成品A-1";
         //        }
         //        else if (craneNO.Contains("3"))
         //        {
         //            Area = "产成品A-1";
         //        }
         //        else if (craneNO.Contains("7"))
         //        {
         //            Area = "产成品C-1";
         //        }
         //        else
        ...
        private void queryStrategyInfo(string carneON)
        {
            ...
            try
            {
                ...
                sql += " B.\"DESC\", B.X_MIN, B.X_MAX, B.Y_MIN, B.Y_MAX, B.BAY_NO  FROM YARD_TO_CAR_CRANE_STRAEGY A ";
        ...
        private bool queryStrategyInfoDetail(string areaID)
        {
            ...
            try
            {
                string sql = " SELECT  A.CRANE_NO, A.ID, A.AREA_ID, A.COIL_STRATEGY_ID, A.FLAG_ENABLED, A.SEQ, B.LIST_UNIT_NO, B.FLAG_FIND_BY_PLAN, ";
                ...
                sql += " LEFT JOIN YARD_TO_YARD_FIND_SADDLE_STRATEGY B ON A.COIL_STRATEGY_ID = B.ID  ";
        ...
        private bool queryStrategyInfoDetail(string areaID, string craneNO)
        {
            ...
            try
            {
                string sql = "  SELECT  A.CRANE_NO, A.ID, A.AREA_ID, A.COIL_STRATEGY_ID, A.FLAG_ENABLED, A.SEQ, B.LIST_UNIT_NO, B.FLAG_FIND_BY_PLAN,  ";
                sql += " B.\"DESC\", B.BAY_NO, B.X_MIN, B.X_MAX, B.Y_MIN, B.Y_MAX, B.X_DIR, B.Y_CENTER, B.MIN_EMPTY_SADDLES ";
                sql += " FROM YARD_TO_CAR_CRANE_STRAEGY A  ";
                sql += " LEFT JOIN YARD_TO_YARD_FIND_COIL_STRATEGY B ON A.COIL_STRATEGY_ID = B.ID    ";
                sql += " WHERE A.AREA_ID = '" + areaID + "' ";
        ...
        private void updataStrategyFlagCarToYard(string ID, int flag)
        {
            try
            {
                string sql = "UPDATE YARD_TO_CAR_CRANE_STRAEGY SET ";
